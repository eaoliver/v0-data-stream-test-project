<script lang="ts">
  ...
  async function processCSVLineByLine(file: File) {
    ...
    while (offset < file.size) {
      ...
      for (const line of lines) {
        ...
        if (lineNumber === 0) {
          ...
          foundCharacteristicNameIndex = headers.findIndex(h => h === 'CharacteristicName');
          
          if (foundResultValueIndex === -1) {
            throw new Error('Column validation failed: Could not find "ResultValue" column in header');
          }
          
          if (foundCharacteristicNameIndex === -1) {
            throw new Error('Column validation failed: Could not find "CharacteristicName" column in header');
          }
          
          resultValueIndex = foundResultValueIndex;
          characteristicNameIndex = foundCharacteristicNameIndex;
          
          console.log('[v0] Found columns - ResultValue:', foundResultValueIndex, 'CharacteristicName:', foundCharacteristicNameIndex);
        } else {
          // Process data row
          ...
          if (characteristicName === 'Temperature, water') {
            ...
            if (!isNaN(resultValue)) {
              sum += resultValue;
              count++;
              console.log('[v0] Line', lineNumber, '- Temperature:', resultValue, '| Running count:', count);
            }
          }
        }
    ...
    if (remainder.trim()) {
      ...
      if (row.length > Math.max(foundResultValueIndex, foundCharacteristicNameIndex)) {
        const characteristicName = row[foundCharacteristicNameIndex];
        const resultValueStr = row[foundResultValueIndex];
        ...
        if (characteristicName === 'Temperature, water') {
          ...
          if (!isNaN(resultValue)) {
            sum += resultValue;
            count++;
            console.log('[v0] Final line - Temperature:', resultValue, '| Final count:', count);
          }
        }
      }
    }
    ...
    average = sum / count;
    recordCount = count;
    console.log('[v0] Processing complete - Average:', average, '| Total records:', count);
  }
